...
SCRIPT_PATH = ROOT / ".github" / "actions" / "changelog-text" / "changelog_to_text.py"
SPEC = importlib.util.spec_from_file_location("changelog_to_text", SCRIPT_PATH)
...
class IdempotenceTests(unittest.TestCase):
    def test_plain_text_passes_through_unchanged(self):
        sample = "2.14.0 (https://example.com/compare/v1...v2) (2026-09-21)\n\nFeatures\n\n- upscaling: flag drift https://example.com/issues/710"
        self.assertEqual(nexus_changelog.markdown_to_plain_text(sample), sample)

    def test_second_pass_matches_first(self):
        body = "## Bug Fixes\n\n* **wind:** preserve buffers ([#726](https://example.com/issues/726))\n"
        once = nexus_changelog.markdown_to_plain_text(body)
        self.assertEqual(nexus_changelog.markdown_to_plain_text(once), once)
